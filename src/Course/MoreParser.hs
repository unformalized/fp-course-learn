...
between ::
  ...
  -> Parser a
  -> Parser a
between begin end resp =
  lift3 (const const) begin resp end
...
hex =
  (\input ->
    let x = readHex input
    in case x of
      Empty -> constantParser (UnexpectedString input)
      Full a -> pure (chr a)
  ) =<< replicateA 4 (satisfy isHexDigit)

-- chr :: int -> Char
-- readHex :: Chars -> Optional Int
--
